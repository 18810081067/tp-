...
<div class="page-container">
	<div class="text-c">
		<form class="Huiform" method="post" action="" target="_self">
			...
			</span><a href="pingpaiadd"><button type="button" class="btn btn-success" id="" name="" onClick="picture_colume_add(this);"><i class="Hui-iconfont">&#xe600;</i> 添加</button></a>
...
<?php foreach ($arr as $key => $v) {?>  
			<tbody>
				<tr class="text-c">
					...
					<td><?=$v['pid'] ?></td>
					<td><img src="<?=$v['pimg'] ?>" alt="" width="50" height="50"></td>
					<td><?=$v['pname'] ?></td>
				<td>
					<?php 
					$dsn = "mysql:host=localhost;dbname=dingcan";
   					$db = new PDO($dsn, 'root', 'root');
   					//var_dump($db)
		 			$rs = $db->query("select sid,sname from ding_shang where sid='$v[sid]'");
		   			$ar = $rs->fetch();
		   			echo $ar['sname'];
 		 			?>								 		
				</td>

					<td class="text-l"><?=$v['pxiang'] ?></td>
					<td><?=$v['ptime'] ?></td>
